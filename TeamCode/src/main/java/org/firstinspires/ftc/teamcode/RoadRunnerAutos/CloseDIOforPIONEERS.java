...
public class CloseDIOforPIONEERS extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Vector2d gate = new Vector2d(3.5728, 60*r);
        ...
        Actions.runBlocking(drive.actionBuilder(drive.localizer.getPose()).strafeToConstantHeading(gate).strafeToConstantHeading(middleSpike).build());
        intake.setPower(-1);
        shooter.spinUp(1320);
        ...
        Actions.runBlocking(
                new ParallelAction(
                        ...
                        updatePose(new Pose2d(closeSpike.x, closeSpike.y + (13*r), Math.toRadians(intakeAngle)))
                )
        );
