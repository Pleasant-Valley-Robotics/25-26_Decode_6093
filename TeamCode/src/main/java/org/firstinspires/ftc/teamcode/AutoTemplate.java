...
public abstract class AutoTemplate extends LinearOpMode {
    ...
    public void setupAuto() {
        ...
        // The rest of the auto should be written in the child class
    }


    public void driveStraightWithOdo(double maxDriveSpeed, double distance, double heading) {
        if (opModeIsActive()) {
            ...
            // Calculate the target position based on the heading direction
            // Note: Don't use Math.abs on start position - negative coordinates are valid!
            double headingRad = Math.toRadians(heading);

            double targetX = startPos.getX(DistanceUnit.INCH) + distance * Math.cos(headingRad);
            double targetY = startPos.getY(DistanceUnit.INCH) + distance * Math.sin(headingRad);
            double driveSpeed;
            ...
            while (opModeIsActive()) {
                ...
                // Calculate distance remaining using Pythagorean theorem
                double deltaX = targetX - currentPos.getX(DistanceUnit.INCH);
                double deltaY = targetY - currentPos.getY(DistanceUnit.INCH);
                double distanceRemaining = Math.sqrt(deltaX * deltaX + deltaY * deltaY);

                if (distanceRemaining <= tolerance) {
                    break;
                }

                // Apply steering correction to maintain heading
                turnSpeed = getSteeringCorrection(heading, P_TURN_GAIN);
                driveSpeed = P_DRIVE_GAIN * distanceRemaining * Math.max(deltaY, deltaX)/Math.abs(Math.max(deltaY, deltaX));
                driveSpeed = Math.min(maxDriveSpeed, driveSpeed);
                moveRobot(driveSpeed, turnSpeed);

                // Telemetry

                telemetry.addData("Target", "X: %.2f, Y: %.2f", targetX, targetY);
                telemetry.addData("Current", "X: %.2f, Y: %.2f",
                        currentPos.getX(DistanceUnit.INCH),
                        currentPos.getY(DistanceUnit.INCH));
                telemetry.addData("Distance Remaining", "%.2f inches", distanceRemaining);
                telemetry.addData("Original Distance", "X: %.2f", distance);
                telemetry.addData("Drive Speed", "X: %.2f", driveSpeed);

                telemetry.addData("Heading", "Target: %.1f, Current: %.1f",
                        heading, currentPos.getHeading(AngleUnit.DEGREES));

                // Convert velocity to inches/sec for clearer display
                telemetry.addData("Velocity (in/sec)", "X: %.1f, Y: %.1f",
    ...
    private void sendTelemetry(boolean straight) {
        ...
        if (straight) {
            ...
            telemetry.addData("Actual Pos L:R",  "%7d:%7d",      frontLeftDrive.getCurrentPosition(), frontRightDrive.getCurrentPosition(), backLeftDrive.getCurrentPosition(), backLeftDrive.getCurrentPosition());
        } else {
